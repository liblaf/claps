...
impl Client {
    pub async fn create(&self, name: &str, addr: &IpAddr) -> Result<DNSRecord> {
        let request = self
            .client
            .post(format!("{}/zones/{}/dns_records", self.api, self.zone))
        ...
        tracing::info!({ name = name, addr = ?addr }, "Create DNS Record");
        Ok(response.result)
    }
}
